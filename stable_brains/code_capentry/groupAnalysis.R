...
coldata <- read_csv("manuscript/brain/sample_table.csv")
head(coldata)

# First AMY data 
amy_data<- coldata %>% filter(region == "AMY")%>% arrange(condition) %>% arrange(group) 

row.names <- amy_data$SampleName

amy_data1 <- amy_data%>% dplyr::select(-SampleName)

amy_data1$Prerank <- as.factor(amy_data1$Prerank)
amy_data1$Postrank <- as.factor(amy_data1$Postrank)
...
pfc_data<- coldata %>% filter(region != "AMY")%>% arrange(condition) %>% arrange(group) 

row.names <- pfc_data$SampleName

pfc_data1 <- pfc_data%>% dplyr::select(-SampleName)

pfc_data1$Prerank <- as.factor(pfc_data1$Prerank)
pfc_data1$Postrank <- as.factor(pfc_data1$Postrank)
...
#filtering genes with counts less than 10 across 90% of samples
x = .90*67

akeep <- rowSums(counts(dag)>= 10) >= x

dag <- dag[akeep,]
nrow(dag) #11321 well that lost a lot. 
...
#filtering 
pkeep <- rowSums(counts(dpg)>= 10) >= x

dpg <- dpg[pkeep,]
nrow(dpg) #11558 more in mpfc but not much


# Normalized counts 
pfc <- estimateSizeFactors(dpg)
...
vsd_pfc <-vst(pfc, blind = T)
vsd_amy <-vst(amy, blind = T)

#Extract the matrix of transformed counts 
vsd_mat_pfc <- assay(vsd_pfc)
vsd_mat_amy <- assay(vsd_amy)

#Compute the correlation values between samples
vsd_cor_pfc <- cor(vsd_mat_pfc)
vsd_cor_amy <- cor(vsd_mat_amy)


#Plot the heatmap based off group and condition

df <- as.data.frame(pfc_data1[,c("group","condition")], row.names=rownames(pfc_data1))

rownames(df) <- colnames(vsd_mat_pfc)

df1 <- as.data.frame(amy_data1[,c("group","condition")], row.names=rownames(amy_data1))

rownames(df1) <- colnames(vsd_mat_amy)

#mpfc by group 
pheatmap(vsd_cor_pfc,
         cluster_rows = FALSE,
         show_rownames = FALSE,
         cluster_cols = FALSE,
         annotation=df)

# probably need to remove that one control point that looks like an outlier. 
# vsd_cor_pfc[, -B1.PFCB12.3.1.trim.sam.counts] 
...
## looks like maybe so outliners in amy as well idk (need to talk to Won or google more before removing)

#Principal component analysis 
#need transform normalized counts which we saved as vsd_pfc and vsd_amy

plotPCA(vsd_pfc, intgroup = "group")
plotPCA(vsd_amy, intgroup = "group")
plotPCA(vsd_pfc, intgroup = "condition")
plotPCA(vsd_amy, intgroup = "condition")
plotPCA(vsd_pfc, intgroup = "Prerank")
plotPCA(vsd_amy, intgroup = "Prerank")
